...
class _DashboardPageState extends State<DashboardPage> {
  ...
  void initState() {
    ...
    _alertTimer = Timer.periodic(
        const Duration(minutes: 1), (_) => _loadAlerts());
    _envTimer = Timer.periodic(
        const Duration(minutes: 10), (_) => _loadEnvironment());
  }
  ...
  Future<void> _loadAlerts() async {
    ...
    try {
      final alerts = await WeatherService.fetchAllAlerts();
      alerts.addAll([
        DisasterAlert(
          id: 'demo_001', type: DisasterType.earthquake,
          title: '地震速報 M5.2',
          description: '震源深度 10km，南投縣仁愛鄉',
          location: '南投縣仁愛鄉',
          time: DateTime.now().subtract(const Duration(minutes: 5)),
          severity: '中度', raw: {},
        ),
        DisasterAlert(
          id: 'demo_002', type: DisasterType.rain,
          title: '大雨特報',
          description: '南投縣山區預計降雨超過 80mm',
          location: '南投縣',
          time: DateTime.now().subtract(const Duration(minutes: 30)),
          severity: '輕度', raw: {},
        ),
        DisasterAlert(
          id: 'demo_003', type: DisasterType.typhoon,
          title: '海上颱風警報',
          description: '輕度颱風強度持續增強中',
          location: '台灣東部海域',
  ...
  Future<void> _loadEnvironment() async {
    try {
      final rainfall = await RainfallService.fetchRainfall();
      final aqi      = await AqiService.fetchNantouAqi();
      if (!mounted) return;
      setState(() {
        _rainfallStations = rainfall;
        _aqiStation       = aqi;
        _rebuildTimeline();
      });
    } catch (e) {
  ...
  void _rebuildTimeline() {
    ...
    for (final alert in _disasterAlerts.take(5)) {
      Color c;
      switch (alert.type) {
    ...
    if (_rainfallStations.isNotEmpty) {
      final st = _rainfallStations.first;
      if (st.stationId != 'MOCK') {
        events.add(TimelineEvent(
          id: 'rainfall_${st.observeTime.millisecondsSinceEpoch}',
          title: '雨量資料更新',
          subtitle: '${st.stationName}｜24h累積 ${st.rainfall24Hr.toStringAsFixed(1)}mm',
          time: st.observeTime, color: kCyan, category: 'system',
        ));
      }
    }
    if (_supplyCount > 0) {
  ...
  Widget _buildSidebar() {
    return Container(
      ...
      decoration: BoxDecoration(
        color: kSidebarBg,
        border: Border(right: BorderSide(color: kBorderBright, width: 1)),
        boxShadow: [
          BoxShadow(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Container(
          ...
          decoration: BoxDecoration(
              border: Border(bottom: BorderSide(color: kBorderBright))),
          child: Row(children: [
            Container(
              width: 38, height: 38,
              decoration: BoxDecoration(
          ...
          child: Row(children: [
            Container(
                ...
                decoration: BoxDecoration(
                    ...
                    boxShadow: [
                      BoxShadow(
                          color: kGreen.withOpacity(.6), blurRadius: 6)
                    ])),
            ...
            Container(
              ...
              decoration: BoxDecoration(
                  gradient: LinearGradient(
                      colors: [kBlue, kCyan.withOpacity(.8)]),
          ...
          child: InkWell(
            ...
            child: Container(
              ...
              decoration: BoxDecoration(
                  ...
                  borderRadius: BorderRadius.circular(8),
                  border: Border.all(
                      color: Colors.redAccent.withOpacity(.22))),
      ...
      child: InkWell(
        ...
        child: AnimatedContainer(
          ...
          decoration: BoxDecoration(
            ...
            borderRadius: BorderRadius.circular(8),
            border: sel
                ? Border.all(color: kCyan.withOpacity(.3))
                : null,
          ),
          child: Row(children: [
            ...
            if (sel)
              Container(
                  ...
                  decoration: BoxDecoration(
                      ...
                      boxShadow: [
                        BoxShadow(
                            color: kCyan.withOpacity(.6), blurRadius: 6)
                      ])),
          ]),
        ),
      ),
              ...
              Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                SizedBox(
                  ...
                  child: Column(children: [
                    ...
                    const SizedBox(height: 16),
                    _EnvironmentCard(
                        stations: _rainfallStations,
                        aqi: _aqiStation,
                        onTap: _loadEnvironment),
                    const SizedBox(height: 16),
                    _buildQuickActions(),
                  ]),
                ),
  ...
  Widget _buildStatsRow() {
    ...
    return Row(children: [
      Expanded(child: _statCard(Icons.people_alt_outlined,
          '$userCount', '用戶總數', kCyan, '↗ +0%', () => _go(1))),
      const SizedBox(width: 12),
      Expanded(child: _statCard(Icons.warning_amber_rounded,
          '$_emergencyCount', '求救事件', Colors.amber, '進行中', () => _go(3))),
      const SizedBox(width: 12),
      Expanded(child: _statCard(Icons.inventory_2_outlined,
          '$_supplyCount', '物資項目', kGreen, '充足', () => _go(4))),
      const SizedBox(width: 12),
      Expanded(child: _statCard(Icons.favorite_border,
          '$healthReportCount', '健康回報', Colors.pinkAccent, '今日', () => _go(5))),
    ]);
  }
    ...
    return InkWell(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          boxShadow: [
            BoxShadow(color: color.withOpacity(.08),
  ...
  Widget _buildQuickActions() {
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        _quickBtn(Icons.people_alt_outlined, '管理災民',
            kCyan, () => _go(2)),
        const SizedBox(height: 8),
        _quickBtn(Icons.warning_amber_rounded, '緊急事件',
            Colors.amber, () => _go(3)),
        const SizedBox(height: 8),
        _quickBtn(Icons.inventory_2_outlined, '物資管理',
            kGreen, () => _go(4)),
        const SizedBox(height: 8),
        _quickBtn(Icons.favorite_border, '健康回報',   // ← 改成健康回報
            Colors.pinkAccent, () => _go(5)),
      ]),
    );
  }
  ...
  Widget _buildStatusStrip() {
    return Container(
      ...
      decoration: BoxDecoration(
          gradient: LinearGradient(
            colors: [kCardBg, kHighlight.withOpacity(.5), kCardBg],
          ),
          borderRadius: BorderRadius.circular(8),
          border: Border.all(color: kBorderBright)),
      child: Row(children: [
  ...
  Widget _statusDot(String t, Color c) => Row(children: [
        Container(
            ...
            decoration: BoxDecoration(
                ...
                boxShadow: [BoxShadow(
                    color: c.withOpacity(.6), blurRadius: 5)])),
  ...
  static Widget _vline() => Container(
      width: 1, height: 20, color: kBorderBright,
...
class _AlertDetailDialog extends StatelessWidget {
  ...
  const _AlertDetailDialog(
      {required this.alert, required this.onClose});
    ...
    return Dialog(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
            ...
            boxShadow: [
              BoxShadow(color: _color.withOpacity(.15),
        ...
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          Padding(
            ...
            child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  _infoRow(Icons.location_on_outlined, '地點',
                      alert.location),
                  const SizedBox(height: 12),
                  _infoRow(Icons.description_outlined, '說明',
                      alert.description),
                  const SizedBox(height: 12),
                  _infoRow(Icons.access_time, '時間',
                  ...
                  Container(
                    ...
                    child: Row(children: [
                      Icon(Icons.info_outline,
                          color: kCyan.withOpacity(.7), size: 14),
                      const SizedBox(width: 8),
                      const Expanded(
                        child: Text(
                            '此通報來自中央氣象署自動發布系統，\n非用戶端求救訊號。',
...
class _NotificationDialog extends StatelessWidget {
  ...
  const _NotificationDialog(
      {required this.alerts, required this.onClose});
    ...
    return Dialog(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
            ...
            boxShadow: [
              BoxShadow(
                  color: kCyan.withOpacity(.08), blurRadius: 30)
            ]),
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          Container(
            ...
            decoration: BoxDecoration(
                border: Border(
                    bottom: BorderSide(color: kBorderBright))),
            child: Row(children: [
                ...
                itemBuilder: (_, i) {
                  ...
                  switch (a.type) {
                    case DisasterType.earthquake:
                      c = Colors.redAccent; icon = Icons.vibration; break;
                    case DisasterType.typhoon:
                      c = Colors.orangeAccent; icon = Icons.air; break;
                    case DisasterType.rain:
                      c = kCyan; icon = Icons.water_drop; break;
                    default:
                      c = kMuted; icon = Icons.warning_amber;
                  }
                  return Container(
                    padding: const EdgeInsets.fromLTRB(14, 12, 14, 12),
                    decoration: BoxDecoration(
                        border: Border(bottom: BorderSide(
                            color: kBorderBright.withOpacity(.5)))),
                    child: Row(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          Container(
    ...
    return Container(
      ...
      child: Stack(children: [
        ...
        Positioned.fill(child: AnimatedBuilder(
            animation: _ctrl,
            builder: (_, __) =>
                CustomPaint(painter: _RadarSweepPainter(_ctrl.value)))),
        // 標題列
        Positioned(
        ...
        // HUD
        const Positioned(left: 14, top: 68,
            child: _HudBox(text: 'FRAME: 1248')),
        const Positioned(left: 120, top: 68,
            child: _HudBox(text: 'FPS: 60')),
        const Positioned(right: 14, top: 68,
            child: _HudBox(text: 'UTC+8 08:32:14')),
        // 地圖主體
        const Positioned(
            left: 14, top: 96, right: 14, bottom: 8,
            child: _MapBody()),
      ]),
    );
  }
...
class _MapBody extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Stack(clipBehavior: Clip.hardEdge, children: [
      ...
      // 避難所標記
      Positioned(left: 360, top: 25,
          child: _dot('埔里鎮公所', const Color(0xFF00E5A8))),
      Positioned(left: 280, top: 75,
          child: _dot('埔里國中活動中心', const Color(0xFF00E5A8))),
      Positioned(left: 390, top: 120,
          child: _dot('宏仁國中體育館', const Color(0xFF00E5A8))),
      Positioned(left: 210, top: 135,
          child: _dot('眉溪水位監測站', const Color(0xFF00B7FF))),
      Positioned(left: 260, top: 170,
          child: _dot('消防局物資站', const Color(0xFF7D5CFF))),
      // 圖例
      Positioned(left: 8, bottom: 6, child: _legendBox()),
      // 座標
      const Positioned(right: 6, bottom: 36,
          child: _HudBox(text: 'COORDS   23.9642°N, 120.9682°E')),
      const Positioned(right: 6, bottom: 6,
          child: _HudBox(text: 'ZOOM   14x   RANGE   2.5km')),
    ]);
  }

  static Widget _c(double s, Color c, double o) => Container(
      width: s, height: s,
      decoration: BoxDecoration(
          shape: BoxShape.circle,
          ...
          children: [
            ...
            const Text('土石流警戒',
                style: TextStyle(color: Colors.white,
                    fontSize: 18, fontWeight: FontWeight.bold)),
            const SizedBox(height: 3),
            const Text('中山路北段 | 中度風險',
                style: TextStyle(
                    color: Color(0xFF4A6580), fontSize: 10)),
            ...
            Row(children: [
              Container(
                  ...
                  decoration: BoxDecoration(
                      ...
                      borderRadius: BorderRadius.circular(3)),
                  child: const Text('LV.2',
                      style: TextStyle(color: Colors.redAccent,
                          fontWeight: FontWeight.bold,
                          fontSize: 9))),
              const SizedBox(width: 6),
              const Text('影響距離: 500m',
                  style: TextStyle(
                      color: Color(0xFF4A6580), fontSize: 9)),
            ]),
          ]));
  ...
  static Widget _legendBox() => Container(
      ...
      child: const Column(
          ...
          children: [
            ...
            _LegendDot(
                color: Color(0xFF00E5A8), text: '避難收容所'),
            _LegendDot(
                color: Color(0xFF7D5CFF), text: '物資集散站'),
            _LegendDot(
                color: Color(0xFF00B7FF), text: '環境監測站'),
          ]));
}
                ...
                child: ListView.builder(
                  ...
                  itemBuilder: (_, i) {
                    ...
                    switch (a.type) {
                      case DisasterType.earthquake:
                        ic = Colors.redAccent;
                        icon = Icons.vibration; break;
                      case DisasterType.typhoon:
                        ic = Colors.orangeAccent;
                        icon = Icons.air; break;
                      case DisasterType.rain:
                        ic = kCyan;
                        icon = Icons.water_drop_outlined; break;
                      default:
                        ic = kMuted;
                    ...
                    return InkWell(
                      ...
                      child: Container(
                        ...
                        decoration: BoxDecoration(
                            ...
                            borderRadius: BorderRadius.circular(8),
                            border: Border.all(
                                color: ic.withOpacity(.15))),
                        child: Row(children: [
                          Container(
                              width: 34, height: 34,
                              decoration: BoxDecoration(
                                  color: ic.withOpacity(.10),
                                  borderRadius:
              ...
              Expanded(
                child: ListView.builder(
                  ...
                  itemBuilder: (_, i) {
                    ...
                    final matchAlert = alerts
                        .where((a) => a.id == e.id)
                        .firstOrNull;
                    return InkWell(
                      borderRadius: BorderRadius.circular(6),
                      onTap: matchAlert != null
                          ? () => onEventTap(matchAlert)
                          : null,
                      child: Padding(
                        padding:
...
class _EnvironmentCard extends StatelessWidget {
  final List<RainfallStation> stations;
  final AqiStation? aqi;
  final VoidCallback onTap;
  ...
  @override
  Widget build(BuildContext context) {
    final st = stations.isNotEmpty ? stations.first : null;
    final offline = st == null ||
        st.stationId == 'MOCK' ||
        st.stationId.isEmpty;
    final rain = offline
        ? '--'
    ...
    return Container(
      ...
      child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 14),
            Row(children: [
              Expanded(child: _envBox(
                  Icons.water_drop_outlined, kCyan,
                  '累積雨量', rain, '過去24小時')),
              const SizedBox(width: 12),
              Expanded(child: _envBox(
                  Icons.air, kGreen, 'AQI 空氣',
                  aqi != null ? '${aqi!.aqi}' : '--',
                  aqi?.status ?? '載入中')),
            ]),
            if (!offline && st != null) ...[
              ...
              Row(children: [
                const Icon(Icons.location_on,
                    color: kMuted, size: 10),
                const SizedBox(width: 4),
                Text('${st.stationName} 測站',
                    style: const TextStyle(
  ...
  static Widget _envBox(IconData icon, Color ic, String label,
      String value, String sub) {
    return Container(
      padding: const EdgeInsets.all(12),
      decoration: BoxDecoration(
          gradient: LinearGradient(
              colors: [ic.withOpacity(.06), Colors.transparent]),
          borderRadius: BorderRadius.circular(8),
      ...
      child: Column(
          ...
          children: [
            ...
            Text(value,
                style: TextStyle(color: ic, fontSize: 26,
                    fontWeight: FontWeight.bold)),
            Text(sub,
                style: const TextStyle(
...
class GridPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawRect(rect,
        Paint()..color = const Color(0xFF020C18));
    final tc =
    ...
    canvas.drawCircle(
        ...
        Paint()
          ..shader = RadialGradient(colors: [
            const Color(0xFFFF2200).withOpacity(.15),
            Colors.transparent
          ]).createShader(Rect.fromCircle(
              center: tc, radius: size.height * 0.45)));
    final sc =
        ...
        Paint()
          ..shader = RadialGradient(colors: [
            kBlue.withOpacity(.12), Colors.transparent
              ...
              center: sc, radius: size.height * 0.4)));
    canvas.drawRect(
        rect,
        Paint()
          ..shader = LinearGradient(
              ...
              colors: [
                Colors.transparent,
                const Color(0xFF010608).withOpacity(.6)
              ]).createShader(rect));
    final gp = Paint()
    ...
    for (double x = 0; x < size.width; x += 36)
      canvas.drawLine(
          Offset(x, 0), Offset(x, size.height), gp);
    for (double y = 0; y < size.height; y += 28)
      canvas.drawLine(
          Offset(0, y), Offset(size.width, y), gp);
    final sp = Paint()
      ..color = kBlue.withOpacity(.06)
      ..strokeWidth = .8;
    for (double y = 0; y < size.height; y += 2.8)
      canvas.drawLine(
          Offset(0, y), Offset(size.width, y), sp);
  }
...
class _RadarSweepPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawRect(
        ...
        Paint()
          ..shader = RadialGradient(
              center: const Alignment(-0.65, 0.1),
              radius: 1.0,
              colors: [
                const Color(0xFF00FF88)
                    .withOpacity(glow * 1.4),
                const Color(0xFF00DD66).withOpacity(glow),
                const Color(0xFF00FF44)
                    .withOpacity(glow * 0.3),
                Colors.transparent
              ],
              stops: const [
                0.0, 0.35, 0.65, 1.0
    ...
    canvas.drawArc(
        cRect, angle - tail, tail, true,
        Paint()
          ..shader = SweepGradient(
              startAngle: angle - tail,
              endAngle: angle,
              colors: [
...
class ShelterItem extends StatelessWidget {
  ...
  const ShelterItem(
      {super.key, required this.name, required this.distance});
